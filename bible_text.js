...
const bibleAbbrMap = {
    ...
    "마": "마태복음", "막": "마가복음", "누": "누가복음", "요": "요한복음", "행": "사도행전",
...
// [3] 원본 텍스트 저장소 (백틱 ` ` 사이에 붙여넣기)
// 주의: ` (물결표시 키) 기호를 지우지 마세요.
const rawData = `
...
`; 
// ▲ 위 rawData 안에 텍스트를 쭉 이어 붙이면 됩니다. 나중에 출애굽기도 저 밑에 이어서 붙이면 됩니다.


// [4] 자동 변환 로직 (손대지 않으셔도 됩니다)
// rawData 문자열을 분석해서 bibleTextData 객체로 만듭니다.
const bibleTextData = {};

(function parseBibleData() {
    const lines = rawData.trim().split('\n');
    const regex = /^([가-힣]+)([0-9]+):([0-9]+)\s+(.*)$/; // 예: 창1:1 내용
    ...
    lines.forEach(line => {
        ...
        if (match) {
            const abbr = match[1];      // 창
            const chapter = match[2];   // 1
            const verse = match[3];     // 1
            const content = match[4];   // 태초에...

            const fullBookName = bibleAbbrMap[abbr]; // 창 -> 창세기

            if (fullBookName) {
                if (!bibleTextData[fullBookName]) bibleTextData[fullBookName] = {};
                if (!bibleTextData[fullBookName][chapter]) bibleTextData[fullBookName][chapter] = {};
                
                bibleTextData[fullBookName][chapter][verse] = content;
            }
        }
    });
})();
